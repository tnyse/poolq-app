...
class _PickedWidgetState extends State<PickedWidget> {
  ...
  Future getGame(context) async {
    ...
    var response =
    await http.get(Uri.parse('https://poolq.app/getnlf/REG${widget.selectedValue}'), headers: {
  ...
  Future getGameWinner(context) async {
    ...
    Provider.of<DataProvider>(context, listen: false);
    var response =
    await http.get(Uri.parse('https://poolq.app/get_winners/REG${widget.selectedValue}'), headers: {
      'Content-Type': 'application/json; charset=UTF-8',
    }).timeout(Duration(seconds: 20));
    var body = json.decode(response.body);
    // print(body);
  ...
  void initState() {
    ...
    _pickrecordStream = FirebaseFirestore.instance.collection('pickrecord').where("week", isEqualTo: "REG${widget.selectedValue}").where("uid", isEqualTo:"${widget.userId}").snapshots();
    getGame(context);
